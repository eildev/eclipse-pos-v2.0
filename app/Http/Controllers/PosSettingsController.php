...
class PosSettingsController extends Controller
{
   ...
   {
      $settingId = $request->input('setting_id');
      $checkboxFields = ['manufacture_date', 'expiry_date', 'bulk_update',  'low_stock_alert', 'product_set_low_stock', 'color_view', 'size_view',];
   ...
   {
      $settingId = $request->input('setting_id');
      $checkboxFields = ['sms_manage', 'invoice_payment', 'link_invoice_payment',  'due_reminder', 'affliate_program','sale_commission', 'courier_management','drag_and_drop'];
      foreach ($checkboxFields as $field) {
         $values[$field] = $request->has($field) ? 1 : 0;
      }
