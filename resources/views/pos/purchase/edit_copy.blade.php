                                            ...
                                            <div class="row align-items-center">
                                                ...
                                                <div class="col-md-8">
                                                    <input type="number" class="form-control carrying_cost"
                                                        ...
                                                        value="{{ $purchase->total_purchase_cost ?? 0 }}" />
                                                </div>
                                            </div>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            function showSelectedVariants() {
                $.ajax({
                    ...
                    success: function(res) {
                        ...
                        if (res.status == 200) {
                            const items = res?.data?.purchase_item;
                            items.forEach(item => {
                                addVariants(item, 1);
                            });
                            updateSLNumbers();
            ...
            function addVariants(variant, quantity) {
                const variantId = variant?.variant_id ? variant?.variant_id : variant.id;
                // console.log("Varaint Id", variantId);
                const quantity1 = quantity || 1;
                const existingRows = $(`.data_row${variantId}`);
                ...
                console.log(variant);
                $('.showData').append(
                    `<tr class="data_row${variantId}">
            <td>

            </td>
            <td class="pruchase_bars">
                <span type="text" class="form-control border-0 product_name${variantId}"  name="product_name[]" readonly style="width:100%; margin: 0; padding: 0; font-size: 12px; white-space: normal; word-wrap: break-word; max-height: 50px; overflow-y: auto;"> ${variant?.product?.name ?? ""}</span>
            </td>
            <td  ${color_view === '0' ? "hidden" : ""}>
                <input type="text" class="color${variantId} form-control border-0"  name="color[]" readonly value="${variant?.variant?.color_name?.name ? variant?.variant?.color_name?.name : variant?.color_name?.name ?? ""}" style="width:60px;margin:0;padding:0;font-size:12px" />
            </td>
            <td ${size_view === '0' ? "hidden" : ""}>
                <input type="text" class="form-control border-0 size${variantId}"  name="size[]" readonly value="${variant?.variant?.variation_size?.size ? variant?.variant?.variation_size?.size : variant?.variation_size?.size ?? ""}" style="width:60px;margin:0;padding:0;font-size:12px" />
            </td>
            <td>
                <input type="hidden" class="variant_id" name="variant_id[]" readonly value="${variantId ?? 0}" />
                <input type="hidden" class="purchase_item_id" name="purchase_item_id[]" readonly value="${variant?.variant_id ? variant.id : null}" />
              <input type="number" class="form-control cost_price${variantId} ${checkPurchaseEdit == 0 ? 'border-0' : ''}" ${checkPurchaseEdit == 0 ? 'readonly' : ''}  name="cost_price[]" onkeyup="calculateTotal();"  value="${variant?.unit_price ?? variant?.cost_price ?? 0}" style="width:60px;margin:0;padding:3px;font-size:12px"/>
            </td>
            <td class="text-start">
               <div class="d-flex align-items-center justify-content-center">
                 <input type="number" variant-id="${variantId}" class="form-control input-small me-3 quantity" onkeyup="calculateTotal();" name="quantity[]"  value="${variant?.quantity ?? quantity1 ?? 0 }"  style="width:60px;margin:0;padding:3px;font-size:12px" /> <span>${variant?.product?.product_unit?.name ?? "pc"}</span>
            ...
            ${purchase_individual_product_discount === "1" ? ` <td>
                                                                                                                                                                                                                                                                                                                <input type="number" class="form-control input-small me-3 variant_discount${variantId}"  name="variant_discount[]"  onkeyup="calculateTotal();" value="${variant?.discount ?? ""}" style="width:60px;margin:0;padding:3px;font-size:12px" />
                                                                                                                                                                                                                                                                                                            </td>`: ""
            }
            <td>
                <input type="number" class="form-control border-0 variant_subtotal${variantId}"  name="total_price[]" readonly value="${variant?.cost_price ?? 0}"  />
            </td>
            ...
            <td>
                <a href="#" class="btn btn-sm btn-danger btn-icon purchase_delete" data-id=${variantId} style="width:60px;margin:0;padding:0;font-size:12px" >
                    <i class="fa-solid fa-trash-can"></i>
                </a>
            </td>
        </tr>`
            ...
            $('#purchaseForm').submit(function(event) {
                ...
                $.ajax({
                    url: '/purchase/update/' + purchase_id,
                    type: 'POST',
                    data: formData,
                    processData: false,
                    contentType: false,
