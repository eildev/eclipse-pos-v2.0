...
//         let discount = purchase_individual_product_discount === "1" ? (parseFloat($('.variant_discount' + variantId).val()) || 0) : 0;

//         let subtotal = (qty * costPrice) - discount;
//         $('.variant_subtotal' + variantId).val(subtotal.toFixed(2));
//         total += subtotal;
//     });
//     // console.log(total);
//     $('.total').val(total.toFixed(2));
//     let discount_type = purchase_hands_on_discount === "1" ? $('.discount_type').val() : null;
//     // console.log(discount_type);

//     if (discount_type === 'fixed') {
//         let handsOnDiscount = purchase_hands_on_discount === "1" ? parseFloat($('.discount_amount').val()) || 0 : 0;
//         let afterDiscount = total - handsOnDiscount;
//         let carrying_cost = parseFloat($('.carrying_cost').val()) || 0;
//         $('.grand_total').val((carrying_cost + afterDiscount).toFixed(2));
//     } else {
//         let handsOnDiscount = purchase_hands_on_discount === "1" ? parseFloat($('.discount_amount').val()) || 0 : 0;
...
//             handsOnDiscount = 0;
//             let discountValue = (total * handsOnDiscount) / 100;
//             let afterDiscount = total - discountValue;
//             let carrying_cost = parseFloat($('.carrying_cost').val()) || 0;
//             // console.log("discountValue", discountValue);
//             let grandTotal = afterDiscount + carrying_cost;
    ...
    function addVariants(variant, quantity) {
        ...
        $('.showData').append(
            ...
            <td class="pruchase_bars">
                <span type="text" class="form-control border-0 product_name${variant.id}"  name="product_name[]" readonly style="width:100%; margin: 0; padding: 0; font-size: 12px; white-space: normal; word-wrap: break-word; max-height: 50px; overflow-y: auto;"> ${variant?.product?.name ?? ""}</span>
            </td>
            ...
            <td>
                <a href="#" class="btn btn-sm btn-danger btn-icon purchase_delete" data-id=${variant.id} style="width:60px;margin:0;padding:0;font-size:12px" >
