...
$(document).ready(function () {
    ...
    function addRow() {
        ...
        const newRow = `
            <tr id="row-${rowCount}" class="data_row">
                <td style="padding: 5px; text-align: left;">
                    <div style="position: relative;">
                        <input type="text"  style="border: 1px solid #176AF3;font-size:12px"
                        ...
                        <div class="search_result m-0" id="search_result_${rowCount}"
                            style="display: none; position: absolute; background: #fff; border: 1px solid #ddd; z-index: 10; left: 0; width: calc(50vw); max-height: 200px; overflow-y: auto;">
                            <table class="table table-bordered table-sm">
                                <thead>
                                    <tr>
                                        <th>Product Name</th>
                                        <th>Stock</th>
                                       ${window.window.authUserSize ? `<th>Size</th>` : ''}
                                       ${window.window.authUserColor ? `<th>Color</th>` : ''}
                                       ${window.authUserCanCostPrice ? `<th>Cost Price</th>` : ''}
                                       ${window.window.authUserB2BPrice  ? `<th>B2B Price</th>` : ''}
                                       ${window.window.authUserB2CPrice  ? `<th>B2C Price</th>` : ''}
                                        <th></th>
                                    </tr>
                                </thead>
                ...
                <td style="padding: 5px 0px; text-align: center;"><input style="width:60px;margin:0;padding:0;font-size:14px ;margin-top:5px ;font-size:12px" name="product_size[]"  type="text" class="form-control  product_size${rowCount}  border-0   size-input" data-row="${rowCount}" readonly /></td>
                <td style="padding: 5px 0px; text-align: center;">
                <input type="hidden" class="total_stock" name="totalStock[]" value="" data-row="${rowCount}">
                <input type="number" class="form-control unit_price  product_price${rowCount} price-input"  name="unit_price[]" data-row="${rowCount}"   ${checkSellEdit == 0 ? "readonly" : ""} ${checkSellEdit == 0 ? "border-0" : "" }   style="width:120px;margin:0;padding:3px;font-size:12px;border: 1px solid #176AF3;"/>
                ${party_ways_rate_kit == 1
                 ? `
                      <select class="form-control unit_price variant_sale_price${rowCount}" name="variant_sale_price[]" style="width:110px;margin:0;padding:3px;font-size:12px;border: 1px solid #176AF3;" data-row="${rowCount}">
                ...
                <td style="padding: 5px 0px; text-align: center;"><input type="number" product-id="${rowCount}" class="form-control qty-input quantity productQuantity${rowCount}" name="quantity[]" style="width:60px;margin:0; padding:3px;font-size:12px;border: 1px solid #176AF3;" data-row="${rowCount}" value="1" /></td>
                ...
                <td style="padding: 5px 0px; text-align: center;"><input type="number" id="productTotal" class="form-control border-0 subtotal-input product_subtotal${rowCount}" name="total_price[]" data-row="${rowCount}" readonly style="width:60px;margin:5px;padding:0;font-size:12px" /></td>
                <td style="padding: 5px 0px; text-align: center;">
    ...
    $(document).on("keyup", ".global-search", function (e) {
        ...
        if (items.length > 0) {
            ...
            if (e.keyCode === 13) { // Enter key
                e.preventDefault();
                const selectedProduct = findDataContainer.find(".highlight .select-product");
        ...
        let customerId = $(".select-customer").val();
            ...
            $.ajax({
                ...
                //  console.log(response.products.salePartyKitPrice);
                    if (
                        response.products &&
                ...
                // error: function (xhr) {
                //     console.error(
                //         `Search request failed for row ${rowId}:`,
                //         xhr.responseText
                //     );
                //     findDataContainer.append(
                //         '<tr><td colspan="3">Error fetching products</td></tr>'
                //     );
                //     searchResultContainer.show();
                // },
            });
...
$(document).on("input", ".qty-input", function() {
        ...
        if ($(this).hasClass("qty-input")) { // Only check stock when qty-input changes
            ...
            if (typeof sale_without_stock !== "undefined" && sale_without_stock == 1) {
                if (totalStock < qty) {
                    toastr.info(`No stock available for this product.Avialabe Stock(${totalStock})`);
                }
            }
        }
    });
...
//////////////////sale without stock //////////////
    ...
    function saveInvoice() {
        ...
        $('tr[class^="data_row"]').each(function () {
           ...
           let unit_price = parseFloat(row.find('input[name="unit_price[]"]').val()) || 0;
          let variant_cost_price = parseFloat(row.find('input[name="variant_cost_price[]"]').val()) || 0;
            ...
            let product_discount  = row.find('input[name="product_discount[]"]').val() || 0
            // let wa_status = row
